...
function extractFirstAndLastDigits(val: string) {
  const firstDigit = val.match(/\d/);
  const lastDigit = val.match(/\d(?=\D*$)/);
  const firstResult = firstDigit ? parseInt(firstDigit[0]) : 0;
  const lastResult = lastDigit ? parseInt(lastDigit[0]) : firstDigit;
  return `${firstResult}${lastResult}`;
}
export function calculateSumOfCalibrations(lines: string[]) {
  let sum = 0;
  lines.forEach((line) => {
    Object.keys(digitMap).forEach(word => {
      const regex = new RegExp(word, 'g');
      line = line.replace(regex, match => String(`${word}${digitMap[word]}${word}`));
    });
    const digits = extractFirstAndLastDigits(line);
    sum += parseInt(digits);
  });
...
const result = calculateSumOfCalibrations(lines);
