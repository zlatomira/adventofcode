...
exports.calculateSumOfCalibrations = void 0;
var lines = [
...
function extractFirstAndLastDigits(val) {
    var firstDigit = val.match(/\d/);
    var lastDigit = val.match(/\d(?=\D*$)/);
    var firstResult = firstDigit ? parseInt(firstDigit[0]) : 0;
    var lastResult = lastDigit ? parseInt(lastDigit[0]) : firstDigit;
    return "".concat(firstResult).concat(lastResult);
}
function calculateSumOfCalibrations(lines) {
    var sum = 0;
    lines.forEach(function (line) {
        Object.keys(digitMap).forEach(function (word) {
            var regex = new RegExp(word, 'g');
            line = line.replace(regex, function (match) { return String("".concat(word).concat(digitMap[word]).concat(word)); });
        });
        console.log("Line:", line);
        var digits = extractFirstAndLastDigits(line);
        console.log("Digits:", digits);
        sum += parseInt(digits);
    });
    return sum;
}
exports.calculateSumOfCalibrations = calculateSumOfCalibrations;
var result = calculateSumOfCalibrations(lines);
console.log("Sum of calibration values:", result);
//# sourceMappingURL=calibrationSum.js.map